...
if (require.main === module) {
    ...
    (async function() {
        ...
        try {
            ...
            const gdocIdx = process.argv.indexOf("--generate-doc");
            if (gdocIdx >= 0) {
                logTitle("info", "Documenting");
                const gdocPath = path.resolve(process.cwd(), process.argv[gdocIdx + 1]);
                const gdocSrcPaths = process.argv.slice(gdocIdx + 2);
                const generateDoc = require(`${gdocPath}/generator`);
                generateDoc(gdocPath, gdocSrcPaths, { indexFileName: "api", header: `# API of ${packageInfo.name}\r\n`, footer: "-------\r\nCopyright ©2022 Season Studio" });
            }
        ...
        return entries.map(_item => {
            if (_item) {
                ...
                for (let cName in defOutput) {
                    (!cName.startsWith("$")) && (output[cName] || (output[cName] = defOutput[cName]));
                }
                output.name || (output.name = path.basename(_item.input, path.extname(_item.input)));
                output.file || (output.file = `${defOutput.$dir}/${path.basename(_item.input)}`);
                for (let cName in defaultConfigs) {
                    (cName !== "output") && (_item[cName] || (_item[cName] = defaultConfigs[cName]));
                }
            }
            return _item;
        });
